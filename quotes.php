<?php
/**
 * Quote browser (saved quotes). Searchable, paginated list with reopen (share
 * link) and re-stamp-to-today's-rate. Public in this trusted-shop deployment —
 * viewing and rate updates need no password. Deleting a quote still requires the
 * admin login (button and handler are gated on sfc_admin_is_logged_in()).
 * All state-changing actions are CSRF-guarded.
 */
...
$b        = SFC_BASE_PATH;
$is_admin = sfc_admin_is_logged_in();   // gates delete only
$csrf     = sfc_admin_csrf_token();      // works for anonymous sessions too
$flash    = null;
...
if ( 'POST' === ( $_SERVER['REQUEST_METHOD'] ?? 'GET' ) ) {
    if ( ! sfc_admin_csrf_valid( $_POST['csrf'] ?? '' ) ) {
        ...
        if ( ! $is_admin ) {
            $flash = array( 'ok' => false, 'message' => 'Inicie sesión para eliminar cotizaciones.' );
        } else {
            try {
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $h( $b ); ?>/assets/admin.css">
</head>
<body class="adm-body">
<header class="adm-header">
    <div><strong>Lab Gráfico</strong> · Cotizaciones</div>
    <nav class="adm-nav">
        <a href="<?php echo $h( $b ); ?>/">Calculadora</a>
        <a href="<?php echo $h( $b ); ?>/admin/index.php">Precios</a>
        <?php if ( $is_admin ) : ?>
            <a href="<?php echo $h( $b ); ?>/admin/logout.php">Cerrar sesión</a>
        <?php else : ?>
            <a href="<?php echo $h( $b ); ?>/admin/login.php">Iniciar sesión</a>
        <?php endif; ?>
    </nav>
</header>
            ...
            <tbody>
                ...
                <?php else : ?>
                    <?php foreach ( $rows as $r ) : ?>
                        <?php $share = $b . '/quote.php?token=' . rawurlencode( $r['share_token'] ); ?>
                        <tr>
